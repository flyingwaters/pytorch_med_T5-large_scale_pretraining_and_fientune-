...
import pandas as pd
import json
from callback import CUDACallback
...
class PyTorchDataModule(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        try:
            input_text, label = sentence.split("\t")
        except:
            print("wrong sentence to split: ", sentence)
        label = str(label.strip())
        source_text = str(input_text)
...
class LightningDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.weight_file_pth = weight_file_pth
    ...
    def train_dataloader(self):
        """ training dataloader """
        # define a sampler strategy to get
        # druncate the too long dataset by 50000
        # random get length/50000*num
        # decrease the too large scale dataset affect the balance
        # item for all_train.csv for its datasets' length druncate from the 50000
        # using weights [] to balance the likehood of dataitem to be draw in the dataloader
        # according the source of dataset to generate the weight list
        with open(self.weight_file_pth, "r") as f:
            weight = json.load(f)
        weight = np.array(weight)
        sampler_t0 = torch.utils.data.WeightedRandomSampler(
            weight, num_samples=80000, replacement=True)
...
class LightningModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # return DeepSpeedCPUAdam(self.parameters(), lr=0.0001, eps=1e-3)
        model = self.model
        ...
        num_warmup_steps = int(
            self.trainer.estimated_stepping_batches * self.hparams.warmup_fraction)
    ...
    def test_epoch_end(self, test_step_outputs):
        _loss = [x.cpu() for x in test_step_outputs]
...
class SimpleT5:
    ...
    def train(
        ...
        trainer = pl.Trainer(
            ...
            accelerator="gpu",
            # strategy= DeepSpeedStrategy(zero_optimization=True ,stage=3,load_full_weights=False,
            #                          min_loss_scale=1,loss_scale=0.0,initial_scale_power=32,hysteresis=2,
            #                          loss_scale_window=1000,contiguous_gradients=True,pin_memory=True,
            #                          offload_optimizer=True,offload_optimizer_device="cpu",
            #                          offload_parameters=True,offload_params_device="cpu",
            #                          partition_activations=True,logging_batch_size_per_gpu=1,
            #                          ),
            strategy=ColossalAIStrategy(),
            logger=loggers,
            callbacks=callbacks,
            max_epochs=max_epochs,
            devices=devices,  # gpu_num
            precision=precision,
